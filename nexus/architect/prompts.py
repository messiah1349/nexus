...
The system prompt is static for the duration of an interview, so it benefits
from Anthropic's ephemeral prompt caching (enabled in `nexus.llm.anthropic`).
"""
...
def build_architect_prompt(default_config: DomainConfig) -> str:
    return ARCHITECT_SYSTEM_TEMPLATE.format(
